...
class SmartMessager:
    ...
    async def start(self):
        ...
        async with ClientsSession([self.account], proxy=self.proxy, basedir=self.basedir) as clients:
            ...
            self.log.bind(username=tg.me.name).info(f'即将预测当前状态下应该发送的水群消息, 但不会实际发送, 仅用于测试.')
        ...
        self.timeline = distribute_numbers(start_timestamp, end_timestamp, msg_per_day, self.min_interval, self.max_interval)
            ...
            async for tg in clients:
                ...
                if answer:
                    if len(answer) > 50:
                        ...
                        if dummy:
                            log.info(f'当前情况下在聊天 "{chat.name}" 中推断可发送水群内容为: [gray50]{truncate_str(answer, 20)}[/]')
                        else:
